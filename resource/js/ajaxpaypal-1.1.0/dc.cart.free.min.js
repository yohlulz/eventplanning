	...
	var a = d.util = {
		...
		base64ToBytes : function(h) {
			...
			for ( var g = [], j = 0, k = 0; j < h.length; k = ++j % 4) {
				...
				g
						.push(((c.indexOf(h.charAt(j - 1)) & (Math.pow(2, -2
								* k + 8) - 1)) << (k * 2))
								| (c.indexOf(h.charAt(j)) >>> (6 - k * 2)));
			}
			return g;
		}
	};
...
(function() {
	...
	var c = f.MD5 = function(i, g) {
		...
		return g && g.asBytes ? h : g && g.asString ? d.bytesToString(h) : a
				.bytesToHex(h);
	};
	c._md5 = function(y) {
		...
		for ( var p = 0; p < k.length; p++) {
			k[p] = ((k[p] << 8) | (k[p] >>> 24)) & 16711935
					| ((k[p] << 24) | (k[p] >>> 8)) & 4278255360;
		}
		k[n >>> 5] |= 128 << (n % 32);
		k[(((n + 64) >>> 9) << 4) + 14] = n;
	...
	DC.cart.Cart = {
		...
		init : function(options) {
			...
			this._container = new DC.cart.ContatinerView(
					{
						...
						onLeftButtonClick : function() {
							if (self._currentSection == "cart") {
								...
								if (self._currentSection == "address") {
									self._changeSection("cart");
								} else {
									if (self._currentSection == "payment") {
										if (self._cartController.needShipping) {
											self._changeSection("address");
										} else {
											self._changeSection("cart");
										}
									}
						...
						onRightButtonClick : function() {
							if (self._currentSection == "cart") {
								...
								if (self._currentSection == "address") {
									self._cartController.address = self._addressView
											.getShipping();
									if (!self._cartController.address) {
										return;
									}
									self
											.onValidateAddress(self._cartController.address);
									self._changeSection("payment");
								}
							}
						}
			...
			this._cartController = new DC.cart.CartController({
				...
				onCheckout : function(cart) {
					...
					self.closeCart();
					setTimeout(function() {
						var stepId = document.getElementById('submit_add_step');
						if(stepId) {
							$('#submit_add_step').trigger("submit");
						}
					  },500);
				}
			});
		...
		_updateCartView : function(cart) {
			...
			if (this._currentSection == "cart") {
				if (cart.count <= 0) {
					...
					this._container.setTitle(DC.cart.lang["label_item_info"]
							.replace("%S", cart.count));
				}
			...
			if (this._widget) {
				var content = $('<div class="amount">'
						+ DC.Currency[cart.currency] + cart.subTotal.toFixed(2)
						+ '</div><div class="badge">'
						+ DC.cart.lang["cartwidget"].replace("%S", cart.count)
						+ "</div>");
				content.hide();
				this._widget.empty();
		...
		_changeSection : function(section) {
			if (section == "cart") {
				...
				this._container.setTitle(DC.cart.lang["label_item_info"]
						.replace("%S", this._cartController.count));
				this._container.rightButton.setVisible(true);
				this._container.setSection(1);
			} else {
				if (section == "address") {
					this._container.setContent(this._addressView);
					var shipping_info = DC.cart.lang["label_shipping_info"]
							.replace("%S",
									DC.Currency[this._cartController.currency]
											+ this._cartController.shipping);
					...
					if (section == "payment") {
						...
						this._container
								.setTitle(DC.cart.lang["label_payment_info"]);
		...
		_updateContainerButton : function() {
			if (this._cartController) {
				if (this._currentSection == "cart") {
					this._container.leftButton
							.setText(DC.cart.lang["button_cancel"]);
					if (this._cartController.needShipping) {
						this._container.rightButton
								.setText(DC.cart.lang["button_shipping"]);
					} else {
						this._container.rightButton
								.setText(DC.cart.lang["button_billing"]);
					}
				} else {
					if (this._currentSection == "address") {
						this._container.leftButton
								.setText(DC.cart.lang["button_cart"]);
						this._container.rightButton
								.setText(DC.cart.lang["button_billing"]);
					} else {
						if (this._currentSection == "payment") {
							if (this._cartController.needShipping) {
								this._container.leftButton
										.setText(DC.cart.lang["button_shipping"]);
							} else {
...
(function(window) {
	DC.cart.Item = {
		...
		init : function(data) {
			...
			if (typeof data.allowMultiple == "string"
					&& (data.allowMultiple.toLowerCase() == "false" || data.allowMultiple
							.toLowerCase() == "no")) {
	...
	DC.cart.CartController = {
				...
				if (this.items[idx]) {
					if (this.items[idx].shipping != undefined) {
						...
						if (this.items[idx].shipping2 != undefined) {
							this.needShipping = true;
							this.shipping += this.items[idx].shipping2
		...
		getURLPaymentButton : function() {
			...
			for ( var idx in this.items) {
				...
				items += '<input type="hidden" name="item_name_' + i
						+ '" value="' + item.name + '">';
				items += '<input type="hidden" name="amount_' + i + '" value="'
						+ item.price + '">';
				items += '<input type="hidden" name="item_number_' + i
						+ '" value="' + item.id + '">';
				items += '<input type="hidden" name="quantity_' + i
						+ '" value="' + item.qty + '">';
				if (item.shipping > 0) {
					items += '<input type="hidden" name="shipping_' + i
							+ '" value="' + item.shipping + '">';
				}
				if (item.shipping2 > 0) {
					items += '<input type="hidden" name="shipping2_' + i
							+ '" value="' + item.shipping2 + '">';
				}
				if (item.weight > 0) {
					items += '<input type="hidden" name="weight_' + i
							+ '" value="' + item.weight * qty + '">';
				}
				i++;
			}
			var address_key = {
			...
			if (this.needShipping && this.address) {
				for ( var n in this.address) {
					address += '<input type="hidden" name="' + address_key[n]
							+ '" value="' + this.address[n] + '">';
				}
			}
			address += '<input type="hidden" name="currency_code" value="'
					+ this.currency + '">';
			for ( var p in this.paypalOptions) {
				if (p != "server") {
					address += '<input type="hidden" name="' + p + '" value="'
							+ this.paymentOptions[p] + '">';
				}
			}
			...
			var button = $('<form id="' + id + '" action="'
					+ this.paymentOptions.server + '" method="post" ' + target
					+ ">" + items + address + this.paymentOptions.submitButton
					+ "</form>");
		...
		getPaypalButton : function() {
			...
			for ( var p in this.paypalOptions) {
				if (p != "server") {
					address += '<input type="hidden" name="' + p + '" value="'
							+ this.paypalOptions[p] + '">';
				}
			}
			if (this.paypalOptions.server == undefined) {
				this.paypalOptions.server = "https://www.paypal.com/cgi-bin/webscr";
			}
			var target = "";
			if (this.openNewCheckOutWindow) {
				target = 'target="_blank"';
			...
			var button = $('<form id="'
					+ id
					+ '" action="'
					+ this.paypalOptions.server
					+ '" method="post" '
					+ target
					+ '><input type="hidden" name="cmd" value="_cart"> <input type="hidden" name="upload" value="1"> <input type="hidden" name="business" value="'
					+ this.paypalOptions.business
					+ '">'
					+ items
					+ address
					+ '<img src="https://www.paypal.com/en_US/i/btn/btn_xpressCheckout.gif" align="left" style="margin-right:7px;"/></form>');
			button.click(function(e) {
				$("#" + id).trigger("submit");
				self.onCheckout(self);
			});
	...
	DC.cart.ContatinerView = {
		...
		createChildren : function() {
			...
			this._step1 = new DC.Text({
				...
				text : '<div class="tag">' + DC.cart.lang["step1"] + "</div> "
						+ DC.cart.lang["step1_text"]
			});
			this._step2 = new DC.Text({
				styles : "step",
				text : '<div class="tag">' + DC.cart.lang["step2"] + "</div> "
						+ DC.cart.lang["step2_text"]
			});
			this._step3 = new DC.Text({
				styles : "step",
				text : '<div class="tag">' + DC.cart.lang["step3"] + "</div> "
						+ DC.cart.lang["step3_text"]
			});
		...
		setSize : function(w, h) {
			var hh = this.defaultHeight
					- $(".cartcontainer > .header").height()
					- $(".cartcontainer > .footer").height()
					- $(".cartcontainer > .title_container").height()
					- $(".cartcontainer > .control_container").height()
					- $(".cartcontainer > .header_container").height();
			if (hh > 0) {
			...
			if (this._content) {
				this._content.setSize(this._contentContainer.width,
		...
		setNeedShipping : function(address) {
			...
			if (address) {
				this._headerWidth = this.innerWidth / 3;
				this._step3.setText('<div class="tag">' + DC.cart.lang["step3"]
						+ "</div> " + DC.cart.lang["step3_text"]);
				this._headerContainer.addChild(this._step1);
				this._headerContainer.addChild(this._step2);
				this._headerContainer.addChild(this._step3);
			} else {
				this._step3.setText('<div class="tag">' + DC.cart.lang["step2"]
						+ "</div> " + DC.cart.lang["step3_text"]);
				this._headerWidth = this.innerWidth / 2;
				this._headerContainer.addChild(this._step1);
				this._headerContainer.addChild(this._step3);
...
(function(window) {
		...
		createChildren : function() {
			...
			this.qty = new DC.TextInput({
				...
				onChange : function(d) {
					...
					self.dispatchEvent(new DC.cart.CartEvent(
							DC.cart.CartEvent.CHANGE, self.data));
				}
			});
			...
			this.delete_button = new DC.Button({
				...
				onClick : function() {
					self.dispatchEvent(new DC.cart.CartEvent(
							DC.cart.CartEvent.REMOVE, self.data));
				}
			});
		},
		createComplete : function() {
		...
		commitProperties : function(prop, value) {
			...
			case "data":
				...
				if (this.data.price == 0) {
					...
					this.price.setText(DC.Currency[this.data.currency]
							+ this.data.price * this.data.qty);
				}
	...
	DC.cart.CartView = {
		...
		commitSize : function() {
			...
			this.list.setXY(0, 0);
			this.infoContainer
					.setXY(0, this.height - this.infoContainer.height);
		},
		...
		update : function(cart) {
			...
			if (cart.count > 0) {
				...
				this.totalLabel.setText(DC.cart.lang["label_total"]
						+ ' <div class="currency">'
						+ DC.Currency[cart.currency]
						+ '</div><div class="amount">'
						+ cart.subTotal.toFixed(2) + "</div>");
				this.totalLabel.ui.css({
					width : "",
					height : ""
				});
				this.updateButton.ui.css({
					right : this.updateButton.width + this.totalLabel.width
							+ 10
				});
...
(function(window) {
	DC.cart.PaymentView = {
		...
		updatePayment : function(cart) {
			var detail = DC.cart.lang["label_item_cost"].replace("%S",
					cart.count).replace("%A",
					DC.Currency[cart.currency] + cart.subTotal)
					+ "<br/>";
			if (cart.needShipping) {
				detail += DC.cart.lang["label_shipping_cost"].replace("%S",
						DC.Currency[cart.currency] + cart.shipping)
						+ "<br/>";
			}
			detail += DC.cart.lang["label_total_cost"].replace("%S",
...
(function(window) {
	$.fn.extend({
		DCAJAXPaypalCart : function(options) {
			var defaults = {
				width : 500,
				height : 400
			};
			...
			return this.each(function() {
				...
				for ( var fn in self.cart) {
					if (typeof self.cart[fn] == "function") {
						var f = function() {
							self.cart[arguments.callee.FUNCTION_NAME].apply(
									self.cart, arguments);
						};
						f["FUNCTION_NAME"] = fn;
						self[fn] = f;
